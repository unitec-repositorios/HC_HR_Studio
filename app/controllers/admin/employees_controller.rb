  ...
  class EmployeesController < Admin::ApplicationController
    ...
    def employee_params
        params.require(:employee).permit(:image, :name, :employee_id_number, :id_number, :birth_date, :gender, :birth_place, :blood_type, :marital_status,
                                        :cellphone, :emergency_contact_name, :email, :emergency_contact_number, :address, :position_id,
                                        :bank_account, :company, :salary, :contract_type, abilities_employees_attributes: [:id, :ability_id, 
